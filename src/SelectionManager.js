window.console && window.console.log && window.console.log('reading javascript source "SelectionManager.js"...');//IF_DEBUG
...
prototype._keydownCanvas = function(e) {
	...
	if (this._consts._NAVKEYS[e.which]) {
		/**
		  ...
		  @version 1.2.3
		  */
		if (evtmgr.triggerInvalid("onBeforeNavigate", [e])) {
			return;
		}
		...
		switch (e.which) {
			case Util.keyMapKeydown.tab:
				if (e.shiftKey) {
					...
					nextCell = this._idxToCell(this._consts._RIGHT, last);
					this.selectCell(nextCell);
				}
				break;
			case Util.keyMapKeydown.enter:
				if (e.shiftKey) {
					nextCell = this._idxToCell(this._consts._UP, last);
					this.selectCell(nextCell);
				}
				else {
					nextCell = this._idxToCell(this._consts._DOWN, last);
					this.selectCell(nextCell);
				}
				break;
			...
			case Util.keyMapKeydown.pgup:
				if (this._options['multiSelectEnabled'] && e.shiftKey) {
					...
					nextCell = this._idxToCell(this._consts._PGUP, last);
					this.selectCell(nextCell);
				}
				break;
			case Util.keyMapKeydown.pgdn:
				if (this._options['multiSelectEnabled'] && e.shiftKey) {
					...
					nextCell = this._idxToCell(this._consts._PGDN, last);
					this.selectCell(nextCell);
				}
				break;
			...
			case Util.keyMapKeydown.home:
				if (this._options['multiSelectEnabled'] && e.shiftKey) {
					...
					nextCell = this._idxToCell(this._consts._HOME, last);
					this.selectCell(nextCell);
				}
				break;
			case Util.keyMapKeydown.end:
				if (this._options['multiSelectEnabled'] && e.shiftKey) {
					...
					nextCell = this._idxToCell(this._consts._END, last);
					this.selectCell(nextCell);
				}
				break;
		}
		...
		/**
		  ...
		  @version 1.3.0
		  */
		evtmgr.trigger("onAfterNavigate", [nextCell], true);
	}
	else {
			...
			for (col in cols) {
				if (cols.hasOwnProperty(col) && col !== "length") {
					...
					/**
					  ...
					  @version 1.1.7
					  */
					evtmgr.trigger(event, args, true);
				}
			}
		}
